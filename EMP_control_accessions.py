...
with open('missing.csv', 'w') as out:
    out.write('ebi_accession\tn_metadata\tn_samples\tn_fna_files\tmissing\n')
    count = 0
    for accession in ebi_accessions:
        ...
        if accession in directories_list:
            n_samples = len(glob.glob(accession + '/*'))
            ...
            if n_metadata == n_sequences and n_metadata == n_fna_files:
                out.write('\t'.join([accession, str(n_metadata), str(n_samples), str(n_fna_files), 'No', '\n']))
            else:
                out.write('\t'.join([accession, str(n_metadata), str(n_samples), str(n_fna_files), 'Yes', '\n']))
